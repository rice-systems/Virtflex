...
gomp_resolve_num_threads (unsigned specified, unsigned count)
{
  ...
  //gomp_error("Enter resolve_num_threads\n");
  //printf("Enter resolve_num_threads\n");
  icv = gomp_icv (false);
  ...
  if (icv->dyn_var)
    {
      //gomp_error("dynamic threads set\n");
      unsigned dyn = gomp_dynamic_max_threads ();
      //printf("Dynamic threads is set to be: %u, gomp_available_cpus: %lu\n", dyn, gomp_available_cpus);
      if (( specified && dyn < max_num_threads) || !specified)
	max_num_threads = dyn;
	...
	max_num_threads = count;
      ...
      //printf("max_num_threads: %d, dyn: %d\n", max_num_threads, dyn);
    }
...
{
  //static unsigned prev_num_threads;
  //static bool first = true;

  //num_threads = gomp_resolve_num_threads (num_threads, 0);
  
  //if num_threads changed, redo memory layout
  //if (first){
  //        first = false;
  //}
  //else{
  //        if (gomp_topo_change && prev_num_threads != num_threads){
  //                if(set_mempolicy(MPOL_NEXTTOUCH, NULL, 0))
  //                      gomp_error("Error when setting mem policy.");
  //		  gomp_topo_change = false;
  //	  }
  //}
  //printf("Parallel region starts, prev_num_threads:%u, num_threads:%u\n",		  prev_num_threads, num_threads);
  //gomp_error("Parallel region starts\n");
  //prev_num_threads = num_threads;
...
GOMP_parallel (void (*fn) (void *), void *data, unsigned num_threads, unsigned int flags)
{
  ...
  static unsigned prev_num_threads;
  static bool first = true;
  static unsigned long long prev_topo_version;
  FILE * fd;

  num_threads = gomp_resolve_num_threads (num_threads, 0);

  /* get topo_verion */
  fd = fopen("/proc/topo_change", "r");  
  if(!fd){
	if(first){
		printf("GOMP: error when open proc file\n");
	}
  }
  else{
	if(fgets((char*)(&gomp_topo_version), 
		sizeof(unsigned long long), fd) != NULL){
		//printf("GOMP: gomp_topo_version %llu\n", gomp_topo_version);
	}
  	fclose (fd);
  }
  ...
  if (first){
          first = false;
	  prev_topo_version = gomp_topo_version;
  }
  else{
	  //gomp_topo_change = (prev_topo_version != gomp_topo_version);
          if (gomp_topo_change && prev_num_threads != num_threads){
          //if (gomp_topo_change){
	  #ifdef DEBUG
		struct timeval stop, start;
		double secs = 0;
		printf("GOMP: prev gomp_topo_version %llu\n", prev_topo_version);
		printf("GOMP: new gomp_topo_version %llu\n", gomp_topo_version);
		gettimeofday(&start, NULL);
  		printf("GOMP: prev_num_threads:%u, num_threads:%u\n",\
			prev_num_threads, num_threads);
	  #endif
		  ...
		  if(gomp_places_list != NULL){
			...
			if (gomp_places_node_list != NULL){
				free(gomp_places_node_list);
				gomp_places_node_list = NULL;
			}
			gomp_places_list_len = 0;
			gomp_init_affinity();
			gomp_reset_affinity = true;
	  #ifdef DEBUG
  			printf("GOMP: reset_affinity = true \n");
	  #endif
		  }
		  //gomp_error("Set next touch flag\n");
                  //if(set_mempolicy(MPOL_NEXTTOUCH, NULL, 0))
                  //      gomp_error("Error when setting mem policy.");
		  gomp_topo_change = false;
		  #ifdef DEBUG
		  ...
		  secs = (double)(stop.tv_usec - start.tv_usec) / 1000000 + (double)(stop.tv_sec - start.tv_sec);
          	printf("Thread reinitiation time taken %f\n",secs);
		  #endif
	  }
  }
  //printf("Parallel region starts, prev_num_threads:%u, num_threads:%u\n",prev_num_threads, num_threads);
