...
{
	//printf("Set thread affinity: cpuset_size: %ld, cpu_set: %ld, sizeof(unsigned long): %ld, places_len: %ld, place: %d\n", gomp_cpuset_size, *(unsigned long *) gomp_places_list[place], sizeof(unsigned long), gomp_places_list_len,  place);
	//printf("Set thread affnity for new thread, place: %d, id: %f., place_len: %ld\n", place, log2( (double)*(unsigned long*) gomp_places_list[place] ), gomp_places_list_len);
  pthread_attr_setaffinity_np (attr, gomp_cpuset_size,
			       (cpu_set_t *) gomp_places_list[place]);
}

int gomp_reset_thread_affinity(pthread_t pt, unsigned int place)
{
	//printf("Set thread affnity for old thread, place: %d, id: %f., place_len: %ld\n", place, log2( (double)*(unsigned long*) gomp_places_list[place] ), gomp_places_list_len);
	return pthread_setaffinity_np(pt, gomp_cpuset_size, 
			(cpu_set_t *) gomp_places_list[place]);
}
			    ...
			    cpu_set_t *copy, char *name, bool quiet)
{
  ...
  for (i = 0; i < max && gomp_places_list_len < count; i++)
	...
	if (getline (&line, &linelen, f) > 0)
	  {
		//printf("start parsing line: %s for cpu: %ld, level:%d this_level: %d\n", line, i, level, this_level);
	    char *p = line;
		...
		for (; first <= last; first++)
		  if (!CPU_ISSET_S (first, gomp_cpuset_size, copy))
		      ...
		      if (gomp_affinity_add_cpus (pl, first, 1, 0, true))
			{
			  ...
			  if (level == 1){
			    gomp_places_node_list[gomp_places_list_len] = 
				    numa_node_of_cpu(first);
			    gomp_places_list_len++;
			    //printf("gomp_places_list_len: %ld\n", gomp_places_list_len);
			  }
			}
		    }
		if (*p == ',')
...
void gomp_affinity_permutate()
{
	int i, t, swap_count = 0;
	void* tmp_set;
	// this is a ad hoc NUMA solution to 4 node machine
//	for (i=0; i < gomp_places_list_len; i++)
//		if (gomp_places_node_list[i] == 1)
//			gomp_places_node_list[i] = 2;
//		else if (gomp_places_node_list[i] == 2)
//			gomp_places_node_list[i] = 1;

	while(1){
		swap_count = 0;
		for (i=0; i < gomp_places_list_len-1; i++){
			if(gomp_places_node_list[i] > gomp_places_node_list[i+1]){
				t = gomp_places_node_list[i];
				gomp_places_node_list[i] = gomp_places_node_list[i+1];
				gomp_places_node_list[i+1] = t;
				tmp_set = gomp_places_list[i];
				gomp_places_list[i] = gomp_places_list[i+1];
				gomp_places_list[i+1] = tmp_set;
				swap_count++;
			}
		}
		if(swap_count == 0)
...
//		errno = 0;
//    		if( sched_getaffinity( 0, gomp_cpuset_size, 
//				gomp_cpusetp) != 0 )	
//	    		printf("Fail to get affinity for the process, errno: %d\n", errno);	
//	//	nproc = sysconf (_SC_NPROCESSORS_ONLN);
//     	//	CPU_ZERO(gomp_cpusetp);
//	//	for (i = 0; i < nproc; i++) {
//        //    		CPU_SET_S(i, gomp_cpuset_size, gomp_cpusetp);
//        //	}
//      	}
      unsigned long maxcount;
	maxcount = gomp_cpuset_popcount (gomp_cpuset_size, gomp_cpusetp);
      //printf("maxcount: %lu, count: %lu\n", maxcount, count);
      if (count > maxcount)
	count = maxcount;
    }
  gomp_places_list = gomp_affinity_alloc (count, quiet);
  gomp_places_node_list = gomp_malloc( count * sizeof(int) );
