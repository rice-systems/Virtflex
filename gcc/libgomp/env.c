...
int *gomp_places_node_list;
unsigned long gomp_places_list_len;
int gomp_debug_var;
unsigned int gomp_num_teams_var;
char *goacc_device_type;
int goacc_device_num;
bool gomp_topo_change = false;
bool gomp_reset_affinity = false;
unsigned long long gomp_topo_version = 0;
#ifndef LIBGOMP_OFFLOADED_ONLY
...
void handle_sigusr(int sig){
    if(sig == SIGUSR1){
    	// set global available threads accordingly
//  gomp_init_num_threads ();
	//gomp_available_cpus=sysconf (_SC_NPROCESSORS_ONLN);
...
//    if (gomp_global_icv.bind_var != omp_proc_bind_false)
//    gomp_init_affinity ();
    gomp_topo_change=true;
        printf("GOMP: Recieved Hotplug signal\n");
	#ifdef DEBUG
	#endif
    }
}
...
initialize_env (void)
{
  ...
  if (!parse_unsigned_long_list ("OMP_NUM_THREADS",
				 ...
				 &gomp_nthreads_var_list_len))
    gomp_global_icv.nthreads_var = gomp_available_cpus;
  //register user define signal handler
    if(signal(SIGUSR1, handle_sigusr) == SIG_ERR)
        gomp_error("Error register signal SIGUSR1\n");
  bool ignore = false;
  if (parse_bind_var ("OMP_PROC_BIND",
		      &gomp_global_icv.bind_var,
		      &gomp_bind_var_list,
