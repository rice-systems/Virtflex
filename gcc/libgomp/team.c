		 ...
		 unsigned flags, struct gomp_team *team)
{
  ...
  nested = thr->ts.level;
  pool = thr->thread_pool;
  //if (gomp_reset_affinity){
  //	pool->threads_used = 0;
  //	gomp_reset_affinity = false;
  //}
  task = thr->task;
      ...
      for (; i < n; ++i)
	{
	  ...
	  if (__builtin_expect (gomp_places_list != NULL, 0))
		...
		nthr = pool->threads[i];
	      place = p + 1;
	      if (gomp_reset_affinity){
		      if( gomp_reset_thread_affinity (nthr->pt, p) != 0)
			gomp_error("Fail to reset affinity of existing threads\n");
                }
	    }
	  else
	    nthr = pool->threads[i];
  ...
  for (; i < nthreads; ++i)
    ...
    if (gomp_reset_affinity){  
  	gettimeofday(&stop, NULL);
    	...
    	printf("Launching new threads taken %f\n",secs);
    }
    #endif
 ...
 do_release:
      if (gomp_reset_affinity){
	      ...
	      struct timeval stop, start;
	      double secs = 0;
	      printf("GOMP: call NEXTTOUCH toreset mem layout\n");
	      gettimeofday(&start, NULL);
	      #endif
      	  //if(set_mempolicy(MPOL_NEXTTOUCH, NULL, 
	  //		MPOL_F_NT_PARALLEL | 
	  //		MPOL_F_NT_DETERMINISTIC))
      	  if(set_mempolicy(MPOL_NEXTTOUCH, NULL, 
			MPOL_F_NT_PARALLEL))
	  	gomp_error("Error when setting mem policy.");
	  #ifdef DEBUG
