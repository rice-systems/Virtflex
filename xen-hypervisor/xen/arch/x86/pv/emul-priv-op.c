                    ...
                    struct x86_emulate_ctxt *ctxt)
{
    ...
    case MSR_K7_EVNTSEL0 ... MSR_K7_PERFCTR3:
        ...
        /* fall through */
    default:
	//printk("In read_msr, in the default tag, %u\n", reg);
	...
	//printk("In read_msr, guest is calling rdmsr_safe, %u\n", reg);
        if ( rdmsr_safe(reg, *val) ){
	    //printk("In read_msr, calling rdmsr_safe, reg %u unsuccessful\n", reg);
            break;
	}
	//printk("In read_msr, returning X86EMUL_OKAY, val %" PRIu64 "\n", *val);
        return X86EMUL_OKAY;
    }
                     ...
                     struct x86_emulate_ctxt *ctxt)
{
    ...
    //printk("In write_msr, begining. reg %u\n", reg);
    ...
    //printk("In write_msr, begining switching, reg %u\n", reg);
    switch ( reg )
    {
        uint64_t temp;
        int rc;
    ...
    ////case 0xc0010015:
    //{
    //    //printk("In write_msr, writing to 0xc0010015/c0011020, reg: %u\n", reg);
    //    if(wrmsr_safe(reg, val) == 0){
    //        printk("In write_msr, writing to 0xc0010015/c0011020, reg: %u succeeded\n", reg);
    //        return X86EMUL_OKAY;
    //    }
    //	break;
    //}
    case 0xc0010230:
    {
    	printk("In write_msr, writing to 0xC0010230, val: %lu\n", val);
	if((wrmsr_safe(reg, val) == 0)){
		printk("In write_msr, writing to 0xC0010230, val: %lu succeeded\n", val);
		return X86EMUL_OKAY;
	}
	break;
    }
    case 0xc0010240 ... 0xc0010247:
    {
	// add support to AMD EPYC Data Fabric counters
	printk("In write_msr, writing to 0xc001024x, reg: %u\n", reg);
	if(wrmsr_safe(reg, val) == 0){
	    printk("In write_msr, writing to 0xc001024x, reg: %u succeeded\n", reg);
	    return X86EMUL_OKAY;
	}
    	break;
    }
    ...
    case MSR_K7_EVNTSEL0 ... MSR_K7_PERFCTR3:
        ...
        /* fall through */
    default:
        //printk("In write_msr, after default tag, reg %u\n", reg);
        ...
        //printk("In write_msr, about to call rdmsr_safe, reg %u\n", reg);
    ...
    //printk("In write_msr, return X86EMUL_UNHANDLEABLE, reg %u\n", reg);
    return X86EMUL_UNHANDLEABLE;
}
