                              ...
                              struct npfec npfec)
{
    ...
    //printk("I'm in hvm_hap_nested_page_fault, vcpuid: %d, paddr_t %lu, domid: %d, w %d, r %d, x %d\n", curr->vcpu_id, gpa, currd->domain_id, npfec.write_access, npfec.read_access, npfec.insn_fetch);
    ...
    {
        int rv;
	//printk("I'm in hvm_hap_nested_page_fault, nested virt part\n");
    ...
    //printk("I'm in hvm_hap_nested_page_fault, after the nested virt part\n");
	...
	//printk("I'm in hvm_hap_nested_page_fault, brefore get to type\n");
    ...
    mfn = get_gfn_type_access(hostp2m, gfn, &p2mt, &p2ma,
                              ...
                              NULL);
    if (p2mt == p2m_migration){
	printk("Page fault type is p2m_migration\n");
	if (atomic_read(&hvm_hap_migration)){
		printk("Page fault type is p2m_migration, pause vCPU\n");
		vcpu_pause(curr);
		while(atomic_read(&hvm_hap_migration));
		mfn = get_gfn_type_access(hostp2m, gfn, &p2mt, &p2ma, 0, NULL);
		vcpu_unpause(curr);
		printk("Page fault type is p2m_migration, unpause vCPU\n");
	}
	else{
		printk("hvm_hap_migration in pgfault: %d\n", atomic_read(&hvm_hap_migration));
	}
    }
    ...
    //printk("I'm in hvm_hap_nested_page_fault, 3\n");
    ...
    //printk("I'm in hvm_hap_nested_page_fault, 4\n");
    ...
    //printk("I'm in hvm_hap_nested_page_fault, 5\n");
    ...
    //printk("I'm in hvm_hap_nested_page_fault, 6\n");
    ...
    //printk("I'm in hvm_hap_nested_page_fault, 7\n");
    ...
    //printk("I'm in hvm_hap_nested_page_fault, 8\n");
 ...
 out_put_gfn:
    ...
    if ( ap2m_active )
        __put_gfn(hostp2m, gfn);
    //printk("I'm in hvm_hap_nested_page_fault, 9\n");
 out:
    ...
    //printk("I'm in hvm_hap_nested_page_fault, exit\n");
    return rc;
}
...
int hvm_msr_read_intercept(unsigned int msr, uint64_t *msr_content)
{
    ...
    if(msr == 0xc001020bU)
    	printk("In hvm_msr_read_intercept, msr %x\n", msr);
    ...
    if ( (ret = guest_rdmsr(v, msr, msr_content)) != X86EMUL_UNHANDLEABLE )
        return ret;
    if(msr == 0xc001020bU)
    	printk("In hvm_msr_read_intercept 1, msr %x\n", msr);
    ...
    case MSR_AMD64_NB_CFG:
     ...
     //case 0xc0010201U ... 0xc001023U:
    case 0xc0010200U ... 0xc001020bU:
    case 0x000000e7U: //MPERF
    case 0x000000e8U: //APERF
    case 0xc00000e9U: //IRPERF
    case 0xc0010064U: // P-state frequency and voltage
    case 0xc0010240U ... 0xc0010247U: // added this category for xen uprof support
    case 0xc0010015U: //Hardware configuration (include IRPERF config)
    {
	printk("In hvm_msr_read_intercept, perfomance counter reg: 0x%x\n", msr);
	if(rdmsr_safe(msr, *msr_content)==0){
		printk("In hvm_msr_read_intercept, read performance counter reg: 0x%x, val: %lu succeeded\n", msr, *msr_content);
	}
	else{ 
		printk("In hvm_msr_read_intercept, read performance counter reg: 0x%x failed\n", msr);
		goto gp_fault;
	}
	break;
    }
    default:
    	//if(msr == 0xc001020bU)
    	printk("In hvm_msr_read_intercept default, msr %x\n", msr);
        if ( (ret = vmce_rdmsr(msr, msr_content)) < 0 )
            goto gp_fault;
        /* If ret == 0 then this is not an MCE MSR, see other MSRs. */
    	if(msr == 0xc001020bU)
		printk("In hvm_msr_read_intercept default after, msr %x\n", msr);
        ret = ((ret == 0)
                            ...
                            bool may_defer)
{
    ...
    HVMTRACE_3D(MSR_WRITE, msr,
               (uint32_t)msr_content, (uint32_t)(msr_content >> 32));
    //printk("In hvm_msr_write_intercept, msr %u.\n", msr);
    ...
    if ( (ret = guest_wrmsr(v, msr, msr_content)) != X86EMUL_UNHANDLEABLE )
        return ret;
    //printk("In hvm_msr_write_intercept 1, msr %u.\n", msr);
    ...
    case 0xc0010015U: //Hardware configuration (include IRPERF config)
    {
        printk("In hvm_msr_write_intercept, writing to reg: 0x%x, val: %lu\n", msr, msr_content);
        if((wrmsr_safe(msr, msr_content) == 0)){
                printk("In hvm_msr_write_intercept, writing to reg: 0x%x, val: %lu succeeded\n", msr, msr_content);
                //return X86EMUL_OKAY;
        }
	else
		goto gp_fault;
    ...
    default:
    	printk("In hvm_msr_write_intercept 1.5, msr %u., default case\n", msr);
    ...
    //printk("In hvm_msr_write_intercept 2, msr %u., return val: %d\n", msr, ret);

    return ret;

gp_fault:
    //printk("In hvm_msr_write_intercept 3, msr %u., write failed\n", msr);
    return X86EMUL_EXCEPTION;
}
