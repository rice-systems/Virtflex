...
static int svm_vmexit_do_cpuid(struct cpu_user_regs *regs, unsigned int inst_len)
{
	...
	printk("In svm_vmexit_do_cpuid, hvm_check_cpuid_faulting failed\n");
...
static int svm_msr_write_intercept(unsigned int msr, uint64_t msr_content)
{
    ...
    //printk("In svm_msr_write_intercept\n");
    switch ( msr )
    {
    case MSR_IA32_SYSENTER_CS:
    case MSR_IA32_SYSENTER_ESP:
...
void svm_vmexit_handler(struct cpu_user_regs *regs)
{
    ...
    //printk("In svm_vmexit_handler, vmcb _general1_intercepts: %" PRIu32 ", _cr_intercepts: %"PRIu32"\n",vmcb->_general1_intercepts, vmcb->_cr_intercepts);
	...
	//printk("In svm_vmexit_handler, VMEXIT_CPUID case\n");
        if ( inst_len == 0 )
            break;

        rc = svm_vmexit_do_cpuid(regs, inst_len);
	printk("In svm_vmexit_handler, VMEXIT_CPUID case, reg->a:%lu, reg->b:%lu, reg->c:%lu, reg->d:%lu\n",regs->rax, regs->rbx, regs->rcx, regs->rdx );

        if ( rc < 0 )
	{
	    printk("In svm_vmexit_handler, VMEXIT_CPUID case, unexpected exit\n");
            goto unexpected_exit_type;
	}
        if ( !rc )
    ...
    case VMEXIT_RDPMC:
    {
        unsigned int inst_len=2;
        uint32_t low, high;
        uint32_t counter = regs->ecx;


        asm volatile("rdpmc" : "=a" (low), "=d" (high) : "c" (counter));

        printk("In svm_vmexit_handler, VMEXIT_RDPMC case, couter %u, high: %u, low:%u\n", counter, high, low);
       //low = 0xdeadbeef;
       //high = 0xdeadbeef;
