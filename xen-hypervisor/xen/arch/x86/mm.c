...
void put_page(struct page_info *page)
{
    ...
    {
	    //printk("I'm here at put_page, might call free_domheap_page\n");
        if ( cleanup_page_cacheattr(page) == 0 ){
            	//printk("I'm here at put_page, about to call free_domheap_page\n");
		free_domheap_page(page);
	}
        else
            gdprintk(XENLOG_WARNING,
...
void put_page_with_order(struct page_info *page, unsigned int page_order)
{
    ...
    //printk("put_page_w_order being called, order:%u\n", page_order);

    for (i = 0; i < (1UL << page_order); i++, page_iter++ ){
	if ( test_and_clear_bit(_PGC_allocated, &page_iter->count_info) ){
	    ...
	    if ( zero_ref_cnt && unlikely((nx & PGC_count_mask) != 0) ){
		zero_ref_cnt = 0;
		printk("In put_page_with_order, zero_ref_cnt became 0, i: %u, ref count is: %lu\n", i, (nx & PGC_count_mask));
	    }
	}
	else{
		zero_ref_cnt = 0;
		printk("In put_page_with_order, zero_ref_cnt became 0 because set PCG_allocated failed, i: %u, ref count is: %lu\n", i, (page_iter->count_info & PGC_count_mask));
	}
    }
	...
	//printk("free_domheap_pages called, with order: %u\n", page_order);
	free_domheap_pages(page, page_order);
    }
    else{
    	//printk("In put_page_with_order, pages ref count is: %lu, order:%u\n", ((page->count_info) & PGC_count_mask), page_order);
    }
}
