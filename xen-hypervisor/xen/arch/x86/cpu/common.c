...
void identify_cpu(struct cpuinfo_x86 *c)
{
    ...
    printk("In identify_cpu, (c->x86_capability[3]) & (1U << 23): %d\n",(((c->x86_capability[3]) & (1U << 23)) != 0));
    printk("In identify_cpu, (c->x86_capability[3]) & (1U << 24): %d\n",(((c->x86_capability[3]) & (1U << 24)) != 0));
    printk("In identify_cpu, (c->x86_capability[3]) & (1U << 28): %d\n",(((c->x86_capability[3]) & (1U << 28)) != 0));
    printk("In identify_cpu, (c->x86_capability[8]) & (1U << 1): %d\n",(((c->x86_capability[8]) & (1U << 1)) != 0));

	for (i = 0; i < FSCAPINTS; ++i)
	{
		// temparary way to enable the 24 and 28 bit (LLC and NB performance counter flag)
		// enabled another bit X86_FEATURE_PERFCTR_CORE (23 bit) to enable core perf counters
		if(i == 3)
			c->x86_capability[i] &= 0x35e9bfffU;	// this enables 23 24 28 bits
			//c->x86_capability[i] &= 0x3569bfffU; // this enables the 24 and 28 bit
		else if (i == 8)
			c->x86_capability[i] &= 0x00001003U;
		else
		c->x86_capability[i] &= known_features[i];
	}
    printk("In identify_cpu after, (c->x86_capability[3]) & (1U << 23): %d\n",(((c->x86_capability[3]) & (1U << 23)) != 0));
    printk("In identify_cpu after, (c->x86_capability[3]) & (1U << 24): %d\n",(((c->x86_capability[3]) & (1U << 24)) != 0));
    printk("In identify_cpu after, (c->x86_capability[3]) & (1U << 28): %d\n",(((c->x86_capability[3]) & (1U << 28)) != 0));
    printk("In identify_cpu after, (c->x86_capability[8]) & (1U << 1): %d\n",(((c->x86_capability[8]) & (1U << 1)) != 0));
