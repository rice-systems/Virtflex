...
static void sanitise_featureset(uint32_t *fs)
{
    ...
    {
        /* Clamp to known mask. */
        // temparary way to enable the 24 and 28 bit (LLC and NB performance counter flag)
	// enabled another bit X86_FEATURE_PERFCTR_CORE (22 bit), to enable core perf counters.
        if(i == 3)
        	fs[i] &= 0x35e9bfffU;	// this enables 23 24 28 bits
        	//fs[i] &= 0x3569bfffU; // this only enables 24 and 28 bit
        else if (i == 8)
		fs[i] &= 0x00001003U;
	else
...
static void __init calculate_host_policy(void)
{
    ...
    *p = raw_cpuid_policy;
    ...
    printk("In calculate_host_policy, (p->extd.e1c) & (1U << 23): %d\n",(((p->extd.e1c) & (1U << 23)) != 0));
    printk("In calculate_host_policy, (p->extd.e1c) & (1U << 24): %d\n",(((p->extd.e1c) & (1U << 24)) != 0));
    printk("In calculate_host_policy, (p->extd.e1c) & (1U << 28): %d\n",(((p->extd.e1c) & (1U << 28)) != 0));
    printk("In calculate_host_policy, (p->extd.e8b) & (1U << 1): %d\n",(((p->extd.e8b) & (1U << 1)) != 0));
    cpuid_featureset_to_policy(boot_cpu_data.x86_capability, p);
    printk("In calculate_host_policy, p->extd.e1c & 1<<22: %d \n", (((p->extd.e1c) & (1U << 22)) != 0) );
    printk("In calculate_host_policy, p->extd.e1c & 1<<23: %d \n", (((p->extd.e1c) & (1U << 23)) != 0) );
    printk("In calculate_host_policy 2, (p->extd.e8b) & (1U << 1): %d\n",(((p->extd.e8b) & (1U << 1)) != 0));
    recalculate_xstate(p);
    recalculate_misc(p);

    printk("In calculate_host_policy 3, (p->extd.e8b) & (1U << 1): %d\n",(((p->extd.e8b) & (1U << 1)) != 0));
    printk("In calculate_host_policy, cpu_has(boot_cpu_data, X86_FEATURE_TOPOEXT): %d \n", cpu_has(&boot_cpu_data, X86_FEATURE_TOPOEXT));
...
static void __init calculate_pv_max_policy(void)
{
    ...
    *p = host_cpuid_policy;
    ...
    printk("In calculate_pv_max_policy, host_featureset[3] & (1U << 23): %d\n", (pv_featureset[3] & (1U << 23)) != 0);
    printk("In calculate_pv_max_policy, host_featureset[3] & (1U << 24): %d\n", (pv_featureset[3] & (1U << 24)) != 0);
    printk("In calculate_pv_max_policy, host_featureset[3] & (1U << 28): %d\n", (pv_featureset[3] & (1U << 28)) != 0);

    for ( i = 0; i < ARRAY_SIZE(pv_featureset); ++i )
    {
	// temparary way to enable the 24/28 bit, LLC and NB perfomance counter
	// enabled another bit X86_FEATURE_PERFCTR_CORE (23 bit) to enable core perf counter.
	if (i == 3)
		pv_featureset[i] &= 0x15a109e3U;
		//pv_featureset[i] &= 0x152109e3U; // this is when 24 and 28 bit are enabled
		//pv_featureset[i] &= 0x156109e3U; // this is when all three bits are enabled
	else
        pv_featureset[i] &= pv_featuremask[i];
    }

    printk("In calculate_pv_max_policy, host_featureset[3] & (1U << 22): %d\n", (pv_featureset[3] & (1U << 22)) != 0);
    ...
    printk("In calculate_pv_max_policy 1.5, host_featureset[3] & (1U << 23): %d\n", (pv_featureset[3] & (1U << 23)) != 0);
    printk("In calculate_pv_max_policy 1.5, host_featureset[3] & (1U << 24): %d\n", (pv_featureset[3] & (1U << 24)) != 0);
    printk("In calculate_pv_max_policy 1.5, host_featureset[3] & (1U << 28): %d\n", (pv_featureset[3] & (1U << 28)) != 0);
    sanitise_featureset(pv_featureset);
    printk("In calculate_pv_max_policy 1.7, host_featureset[3] & (1U << 23): %d\n", (pv_featureset[3] & (1U << 23)) != 0);
    printk("In calculate_pv_max_policy 1.7, host_featureset[3] & (1U << 24): %d\n", (pv_featureset[3] & (1U << 24)) != 0);
    printk("In calculate_pv_max_policy 1.7, host_featureset[3] & (1U << 28): %d\n", (pv_featureset[3] & (1U << 28)) != 0);
    cpuid_featureset_to_policy(pv_featureset, p);
    printk("In calculate_pv_max_policy 2, host_featureset[3] & (1U << 22): %d\n", (pv_featureset[3] & (1U << 22)) != 0);
    printk("In calculate_pv_max_policy 2, host_featureset[3] & (1U << 23): %d\n", (pv_featureset[3] & (1U << 23)) != 0);
    printk("In calculate_pv_max_policy 2, host_featureset[3] & (1U << 24): %d\n", (pv_featureset[3] & (1U << 24)) != 0);
    printk("In calculate_pv_max_policy 2, host_featureset[3] & (1U << 28): %d\n", (pv_featureset[3] & (1U << 28)) != 0);
    recalculate_xstate(p);
...
static void __init calculate_hvm_max_policy(void)
{
    ...
    *p = host_cpuid_policy;
    ...
    printk("In calculate_hvm_max_policy, host_featureset[3] & (1U << 23): %d\n", (hvm_featureset[3] & (1U << 23)) != 0);
    printk("In calculate_hvm_max_policy, host_featureset[8] & (1U << 1): %d\n", (hvm_featureset[8] & (1U << 1)) != 0);
    for ( i = 0; i < ARRAY_SIZE(hvm_featureset); ++i )
    {
	// temparary way to enable the topoext flag (22 bit) and LLC NB perf counter flags (24 and 28 bit)
	// enabled another bit: X86_FEATURE_PERFCTR_CORE (23 bit) to enable core perf counter.
	if (i == 3)
		hvm_featureset[i] &= 0x15a189f7U; //0x042189f7U is the original value
	else if( i == 8 )
		hvm_featureset[i] &= ( hvm_featuremask[i] | 1<<1U ); // enable the irperf bit
	else
        hvm_featureset[i] &= hvm_featuremask[i];
    }

    printk("In calculate_hvm_max_policy 2, host_featureset[3] & (1U << 22): %d\n", (hvm_featureset[3] & (1U << 22)) != 0);
    printk("In calculate_hvm_max_policy 2, host_featureset[8] & (1U << 1): %d\n", (hvm_featureset[8] & (1U << 1)) != 0);
    /*
    ...
    printk("In calculate_hvm_max_policy 3, host_featureset[3] & (1U << 22): %d\n", (hvm_featureset[3] & (1U << 22)) != 0);
    printk("In calculate_hvm_max_policy 3, host_featureset[3] & (1U << 23): %d\n", (hvm_featureset[3] & (1U << 23)) != 0);
    printk("In calculate_hvm_max_policy 3, host_featureset[8] & (1U << 1): %d\n", (hvm_featureset[8] & (1U << 1)) != 0);
                 ...
                 uint32_t subleaf, struct cpuid_leaf *res)
{
    ...
    //if(leaf == 0x6 || leaf == 0x80000008U)
    	printk("In guest_cpuid, vcpu: %d, leaf: 0x%x, subleaf: 0x%x\n", v->vcpu_id, leaf, subleaf);
    *res = EMPTY_LEAF;
    ...
    case 0 ... CPUID_GUEST_NR_BASIC - 1:
        ...
        case 0x4:
            ...
            *res = p->cache.raw[subleaf];
            break;
	case 0x6:
	    // Added this case to temporarily enables MPERF and APERF bit 
	    // (indicated by lowest bit of ecx)
        ...
        default:
            *res = p->basic.raw[leaf];
    	    if(leaf == 0x6 || leaf == 0x80000008U)
    		printk("In guest_cpuid, vcpu: %d, leaf: %u, subleaf: %u, res->ecx: %u\n", v->vcpu_id, leaf, subleaf, res->c);
            break;
        }
        break;
	...
	// temparorily returning 1 for 0x80000007 edx 10th bit
	if (leaf == 0x80000007U){
		res->d |= (1<<10);
	}
	if (leaf == 0x80000008U){
    		//printk("In guest_cpuid, vcpu: %d, leaf: %u, subleaf: %u, res->b\n", v->vcpu_id, leaf, subleaf);
		printk("In guest_cpuid, case 0x80000008, subleaf: %u, pv? %d, vcpu: %d, a:%u, b:%u c:%u, d:%u\n", subleaf, is_pv_domain(d) , v->vcpu_id, res->a, res->b, res->c, res->d);
	}
        break;
