...
atomic_t hvm_hap_migration = ATOMIC_INIT(0);
...
void p2m_unlock_and_tlb_flush(struct p2m_domain *p2m)
{
    if ( p2m->need_flush ) {
        ...
        printk("I'm here in p2m_unlock_and_tlb_flush, about to flush TLB\n");
	p2m->tlb_flush(p2m);
    } else
        mm_write_unlock(&p2m->lock);
}
...
int guest_migrate_page_range(struct domain *d, gfn_t start_gfn, gfn_t end_gfn, int move_to_node){
	...
	struct p2m_domain *p2m = p2m_get_hostp2m(d);
        ...
        rc = p2m->migrate_page_range(p2m, start_gfn, end_gfn, move_to_node);

        p2m->defer_nested_flush = 0;
        if ( nestedhvm_enabled(d) )
                p2m_flush_nestedp2m(d);
	//p2m->tlb_flush(p2m);
	printk("p2m defer_flush: %d, need_flush %d\n", p2m->defer_flush, p2m->need_flush);
        p2m_unlock(p2m);// tlb flush code p2m_unlock_and_tlb_flush() is called in unlock()
        return rc;
}
