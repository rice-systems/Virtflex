...
#include <xen/event.h>
#include <xen/trace.h>
#include <xen/domain_page.h> // added
#include <xen/mm.h> // added
#include <xen/event_fifo.h> //added
#include <public/vm_event.h>
#include <asm/domain.h>
#include <asm/page.h>
#include <asm/paging.h>
#include <asm/p2m.h>
#include <asm/atomic.h> // added
#include <asm/flushtlb.h> // added
...
//static int p2m_pt_set_migration_range(struct p2m_domain *p2m,
//                                   unsigned int level,
//                                   unsigned long first_gfn,
//                                   unsigned long last_gfn)
//{
...
//    unmap_domain_page(table);
//
//    printk("p2m_pt_set_migration_range count level %u entry: %lu\n", level, present_cnt);
//    return err;
//}

typedef struct migrange_info{
	int dest_node;
	int idx;
	int size;
...
static unsigned long* avoid_mfns;
static int avoid_size;
static int first_pte;

static bool mfn_in_avoid_list(unsigned int mfn, int order)
{
	int i;
	for(i=0; i<avoid_size; i++){
		if(avoid_mfns[i] >= mfn && avoid_mfns[i] <= mfn+(1 << order) -1){
			return true;
			}
	}
	return false;
...
static int p2m_pt_migrate_range_helper(struct p2m_domain *p2m,
					void *table, unsigned long start_gfn,
					unsigned int* iter_level,
					unsigned long* valid_cnt,
					unsigned int level,
					bool migration,
					migrange_info* migrange,
					int move_to_node)
{
	unsigned int i;
	void* table_next=table;
    	l1_pgentry_t *pent;
    	int err = 0;
    	unsigned long remainder = start_gfn;
	l1_pgentry_t e;
	unsigned long mfn, curr_gfn;	
	...
	/* iterate over all entries at this level */
    	for(i=0; i< iter_level[level]; i++){
		
		remainder = start_gfn + (1 << (level) * PAGETABLE_ORDER)*i;
		
    		//printk("examing pages %lu at level: %d, iteration: %d, valid_cnt: %lu\n", 			
		//		remainder, level,iter_level[level], valid_cnt[level] );
		pent = p2m_find_entry(table, &remainder, start_gfn,
                              level * PAGETABLE_ORDER, 1 << PAGETABLE_ORDER);
		remainder = start_gfn + (1 << (level) * PAGETABLE_ORDER)*i;
		if ( !pent )
        	{
			//if(level == 0)
    			//printk("\tpages %lu return null at level: %d\n", remainder, level);
            		continue;
        	}
        	if ( !(l1e_get_flags(*pent) & _PAGE_PRESENT) ){ 
    			//printk("\tpages %lu not present at level: %d\n", remainder, level);
			continue;
		}
		if (level >= 1){
			/*check if it's super page*/
			if( l1e_get_flags(*pent) & _PAGE_PSE){
				//l1_pgentry_t e;

				l2_pgentry_t l2e;

				/* if first_pte is set, skip a few entries */
				if (first_pte > 0){
					first_pte--;
					/* get the skipped gfn number */
					mfn = l1e_get_pfn(*pent);
					printk(" first_pte: skipped entry: gfn: %lu, mfn: %lu, level: %d\n",
						get_gpfn_from_mfn(mfn), mfn, level);
					continue;
				}

				mfn = l1e_get_pfn(*pent);

				/* check if it is on avoid_mfn list, if yes, skip */
				if (mfn_in_avoid_list(mfn,level*PAGETABLE_ORDER)){
					printk("skipped mfn: %lu level: %d, in avoid_mfns list\n", mfn, level);
					continue;
				}

				/* if migration is not set, just mark the entries */
				if (!migration){
					//if (p2m_flags_to_type(l1e_get_flags(*pent)) != p2m_ram_rw)
					//	printk("p2m type is not p2m_ram_rw: %d\n", p2m_flags_to_type(l1e_get_flags(*pent)));
					l2e =p2m_l2e_from_pfn(mfn_x(mfn),
                               			p2m_type_to_flags(p2m, 
						//p2m_flags_to_type(l1e_get_flags(*pent)) & ~_PAGE_RW, mfn, level));
						p2m_migration, mfn, level));
					
					valid_cnt[level]++;
				}
				/* if migration is set, copy and update new mfn*/
				else{
					flags = 0;
					if (move_to_node < 0){
						flags |= (MEMF_exact_node | MEMF_node(
						p2m->domain->vnuma->vnode_to_pnode[0]));
					}
					else{
						if (move_to_node < p2m->domain->vnuma->nr_vnodes){
						flags |= (MEMF_exact_node | MEMF_node(
                                                p2m->domain->vnuma->vnode_to_pnode[move_to_node]));
						//printk("In p2m_pt_migrate_range_helper, move_to_node: %d", move_to_node);
						}
						else{
							printk("In p2m_pt_migrate_range_helper, move_to_node invalid\n");
						}
					}
					new_page = alloc_domheap_pages(p2m->domain, 
							level * PAGETABLE_ORDER,
							flags);
					if (!new_page){
						new_mfn = mfn;
						printk("In p2m_pt_migrate_range_helper, alloc page failed, use old page\n");
					}
					else{
						//printk("new page is valid, level >=1, curr_gfn from m2p, trust me it's brand new %lu\n", 
						//		get_gpfn_from_mfn(mfn));
						ASSERT(migrange->idx < migrange->size);

						/* record the old mfn in migrange->mfn_list */
						migrange->mfn_list[migrange->idx] = mfn;
						migrange->order_list[migrange->idx] = 
								level * PAGETABLE_ORDER;
						migrange->idx++;
						new_mfn = page_to_mfn(new_page);
						//new_mfn = mfn;
						//printk("new mfn: %lu, mfn %lu\n", new_mfn, mfn);

						/* get the gfn from m2p table */
						curr_gfn = get_gpfn_from_mfn(mfn);

						for (j = 0; j < 1<<(level * PAGETABLE_ORDER); j++){
							copy_domain_page(new_mfn+j, mfn+j);
							set_gpfn_from_mfn(mfn+j, INVALID_M2P_ENTRY);
							set_gpfn_from_mfn(new_mfn+j, curr_gfn+j);
						}
						/* in this version, free page right after a successful migration */
						free_domheap_pages(mfn_to_page(mfn), level * PAGETABLE_ORDER);
						
						/* below is some debug code to check the copied content */
						//for (j = 0; j < 1<<(level * PAGETABLE_ORDER); j++){
						//	src = (char*)map_domain_page(mfn+j);
						//	dst = (char*)map_domain_page(new_mfn+j);
						//	for(k = 0; k < PAGE_SIZE/sizeof(char); k++){
						//		if(*(src+k) != *(dst+k)){
						//			printk("Content different\n");
						//		}
						//	}
						//	unmap_domain_page((void *)src);
						//	unmap_domain_page((void *)dst);
						//}	
						//printk("Content same\n");
					}

					l2e =p2m_l2e_from_pfn(mfn_x(new_mfn),
					//l2e =p2m_l2e_from_pfn(mfn_x(mfn),
                               			p2m_type_to_flags(p2m, 
						p2m_ram_rw, new_mfn, level));
					
				}
				//printk("Original p2m flag l2/3: %d\n",(int)p2m_flags_to_type(l1e_get_flags(*pent)));
				e.l1 = l2e.l2;
				p2m->write_p2m_entry(p2m, remainder, pent, e, level);
				continue;	
			}

			/* if it's not superpage, go to the next level*/
        		err = p2m_next_level(p2m, &table_next, 
			     &remainder, start_gfn,
                             level * PAGETABLE_ORDER, 1 << PAGETABLE_ORDER,
                             level, 0);
        		if ( err ){
    				//printk("\tpages %lu next level get err at level: %d\n", remainder, level);
				continue;
			}
			//remainder = start_gfn + (1 << (level) * PAGETABLE_ORDER)*i;
			p2m_pt_migrate_range_helper(p2m, table_next, remainder,
					iter_level, valid_cnt,
					level-1, migration, migrange, move_to_node);
			/* restore table pointer */
			table_next = table;
		}
		/* else if we are at 4k level, do not check the super page bit */
		else{ // level == 0
			
			/* skip entries if first_pte set */
			if (first_pte > 0){
                           	first_pte--;
                        	continue;
                       	} 
			mfn = l1e_get_pfn(*pent);
			/* skip entries if in avoid_mfn list */
			if (mfn_in_avoid_list(mfn,level*PAGETABLE_ORDER)){
				printk("skipped mfn: %lu level: %d, in avoid_mfns list\n", mfn, level);
				continue;
			}
			/* if migration not set, just mark the pte entries */
			if (!migration){
				e =p2m_l1e_from_pfn(mfn_x(mfn),
                               		p2m_type_to_flags(p2m, 
						p2m_migration, mfn, level));
				//p2m_flags_to_type(l1e_get_flags(*pent)) & ~_PAGE_RW, mfn, level));
				//printk("Original p2m flag l1: %d\n",(int)p2m_flags_to_type(l1e_get_flags(*pent)));
				valid_cnt[level]++;
			}
			/* if migration set, copy over and update pte */
			else{
					...
					if (!new_page){
						new_mfn = mfn;
						printk("In p2m_pt_migrate_range_helper, alloc page failed, use old page level 0\n");
					}
					else{
						//printk("new page is valid, level 0, curr_gfn from m2p, trust me it's brand new %lu\n", get_gpfn_from_mfn(mfn));
						ASSERT(migrange->idx < migrange->size);
						/* record old mfn in migrange->mfn_list */
						migrange->mfn_list[migrange->idx] = mfn;
						migrange->order_list[migrange->idx] = 
								level * PAGETABLE_ORDER;
						migrange->idx++;
						new_mfn = page_to_mfn(new_page);
						//new_mfn = mfn;
						//printk("new mfn level 0: %lu, mfn %lu\n", new_mfn, mfn);
						/* get gfn from m2p table */
						curr_gfn = get_gpfn_from_mfn(mfn);
						for (j = 0; j < 1<<(level * PAGETABLE_ORDER); j++){
							copy_domain_page(new_mfn+j, mfn+j);
							set_gpfn_from_mfn(mfn+j, INVALID_M2P_ENTRY);
						...
						/* In this version, try to free the page right after a sucessfull copy */
						free_domheap_pages(mfn_to_page(mfn), level * PAGETABLE_ORDER);
						/* below is some debug code to check the copied content*/
						//for (j = 0; j < 1<<(level * PAGETABLE_ORDER); j++){
						//	src = (char*)map_domain_page(mfn+j);
						...
						//	unmap_domain_page((void *)dst);
						//}	
						//printk("Level 0 Content same\n");
					}

					e =p2m_l1e_from_pfn(mfn_x(new_mfn),
                        	       		p2m_type_to_flags(p2m, 
						p2m_ram_rw, new_mfn, level));
...
static int p2m_pt_migrate_page_range_2(struct p2m_domain *p2m,
                                          unsigned long first_gfn,
                                          unsigned long last_gfn,
					  int move_to_node)
{
    ...
    unsigned int iter_level[3];
    unsigned long valid_cnt[3];
    void* table;
    ...
    /* initialize the count array and the iteration array */ 
    for (i = 0 ; i < 3; i++){ 
	iter_level[i] = 1 << PAGETABLE_ORDER;
	valid_cnt[i] = 0;
    }
    /* find the level at where the first and last gfn are different */
    for (i = 2; i >=0; i --){
   	if (first_gfn >> (i* PAGETABLE_ORDER) 
		== last_gfn >> (i* PAGETABLE_ORDER)){
			iter_level[i] = 1;
			continue;
		}
	else
    ...
    diff_level = i;
    iter_level[diff_level] = (last_gfn >> (diff_level* PAGETABLE_ORDER))
				- (first_gfn >> (diff_level* PAGETABLE_ORDER));

    /* round down first_gfn */
    first_gfn = first_gfn & (~((1 << diff_level * PAGETABLE_ORDER) -1));
    printk("first_gfn and last_gfn is different at level: %d, first gfn %lu\n"
	, i,first_gfn);
    printk("move_to_node: %d\n", move_to_node);

    /* filled avoid_mfns list,
	right now there are two mfn per vcpu that needs to avoid migration,
	1. vcpu_info_mfn is the mfn guest use to store vcpu_info
	2. hypervisor also cache event channel control block per vcpu
	*/
    avoid_size = p2m->domain->max_vcpus * 2;
    avoid_mfns = xmalloc_bytes(sizeof(unsigned long)* avoid_size);
    if (!avoid_mfns){
	avoid_size = 0;
	printk("allocating avoid_mfns failed, avoid nothing\n");
    }

    for(v = 0; v< p2m->domain->max_vcpus; v++){
    	printk("vcpu: %d, vcpu_info_mfn: %lu\n",v, p2m->domain->vcpu[v]->vcpu_info_mfn);
	avoid_mfns[2*v] = p2m->domain->vcpu[v]->vcpu_info_mfn;
	avoid_mfns[2*v+1] = _mfn(virt_to_mfn(p2m->domain->vcpu[v]->evtchn_fifo->control_block));
    	printk("vcpu: %d, evtchn_fifo mfn: %lu\n",v, avoid_mfns[2*v+1]);
    }
    //printk("start_info.store_mfn: %lu, start_info.console.mfn\n", shared_info(p2m->domain, store_mfn)/*, shared_info(p2m->domain, console).domU.mfn*/);
    gfn_remainder = first_gfn;

    rc = p2m_next_level(p2m, &table, &gfn_remainder, first_gfn,
                        L4_PAGETABLE_SHIFT - PAGE_SHIFT,
                        L4_PAGETABLE_ENTRIES, 3, 1);
    if ( rc ){
        printk("Cound not find the highest level, exit\n");
	return -1;
    }

    /* set hvm_hap_migration = 1; */
    while(1)
    {
        old = atomic_read(&hvm_hap_migration);
        new = old + 1;
        if ( likely(old == atomic_cmpxchg(&hvm_hap_migration, old, new)) )
            break; // success!
    }
    printk("hvm_hap_migration value: %d\n", atomic_read(&hvm_hap_migration));
    
    /* first_pte is used to skip the first N valid entries, for debugging use */
    first_pte = 0;

    p2m_pt_migrate_range_helper(p2m, table, gfn_remainder, 
				iter_level, valid_cnt, 2, false, NULL, -1);

    /* count how many valid entries are found */
    valid_total = 0;
    for(i=2; i>=0; i--){
	valid_total += valid_cnt[i];
    	printk("On level %u, migrate page range found %lu valid pages iter_level %u\n",i, valid_cnt[i], iter_level[i]);
	if(i == 0 )
		break;
    }

    /* flush cache and TLB, cache flush is probably not strictly required */
    //flush_all(FLUSH_CACHE);
    flush_area_all( NULL, FLUSH_TLB_GLOBAL | FLUSH_ORDER(2 * PAGETABLE_ORDER));

    /* fill out migrange struct to do the actual migration */
    migrange.dest_node = 0;
    migrange.idx = 0;
    migrange.size = valid_total;
    migrange.mfn_list = xmalloc_bytes(sizeof(unsigned long)* valid_total);
    ASSERT(migrange.mfn_list);
    migrange.order_list = xmalloc_bytes(sizeof(int) * valid_total);
    ASSERT(migrange.order_list);

    /* Second pass, do the copy and migration here */
    first_pte = 0;
    printk("Second pass of pgtable scanning, this is real\n");
    p2m_pt_migrate_range_helper(p2m, table, gfn_remainder,
                                iter_level, valid_cnt, 2, true, &migrange, 
				move_to_node);
	
    /* flush tlb again and free old pages here TODO */
    flush_all(FLUSH_CACHE);
    flush_area_all( NULL, FLUSH_TLB_GLOBAL | FLUSH_ORDER(2 * PAGETABLE_ORDER));
    
    // for each in migrange.mfn_list
    // get page_info struct   
    //free_domheap_pages(new_page, level * PAGETABLE_ORDER); 
    printk("freeing old pages from migrange.mfn_list, d->tot_pages:%d\n", p2m->domain->tot_pages);
    //for(i=0; i< migrange.idx; i++){
	//free_domheap_pages(mfn_to_page(migrange.mfn_list[i]), migrange.order_list[i]);
    //}
    printk("after freeing old pages from migrange.mfn_list, d->tot_pages:%d\n", p2m->domain->tot_pages);

    /* set hvm_hap_migration = 0; this will allow trapped page faults to continue */
    while(1)
    {
        old = atomic_read(&hvm_hap_migration);
        new = old - 1;
    ...
    xfree(migrange.mfn_list);
    xfree(migrange.order_list);
    xfree(avoid_mfns);
...
//static int p2m_pt_migrate_page_range(struct p2m_domain *p2m,
//                                          unsigned long first_gfn,
//                                          unsigned long last_gfn)
//{
//	
...
//    ASSERT(hap_enabled(p2m->domain));
//    
//    printk("p2m_pt_migrate_page_range is called\n");
//
//    for ( i = 1; i <= 4; )
//    {
//        if ( first_gfn & mask )
...
//            unsigned long end_gfn = min(first_gfn | mask, last_gfn);
//
//            err = p2m_pt_set_migration_range(p2m, i, first_gfn, end_gfn);
//            if ( err || end_gfn >= last_gfn )
//                break;
//            first_gfn = end_gfn + 1;
//        }
...
//            unsigned long start_gfn = max(first_gfn, last_gfn & ~mask);
//
//            err = p2m_pt_set_migration_range(p2m, i, start_gfn, last_gfn);
//            if ( err || start_gfn <= first_gfn )
//                break;
//            last_gfn = start_gfn - 1;
//        }
...
void p2m_pt_init(struct p2m_domain *p2m)
{
    ...
    p2m->migrate_page_range = p2m_pt_migrate_page_range_2;
