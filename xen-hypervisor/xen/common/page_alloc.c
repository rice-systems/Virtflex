    ...
    struct domain *d)
{
         ...
         (scrub_debug && !(memflags & MEMF_no_scrub)) )
    {
		...
		{
			scrub_one_page(&pg[i]);
			printk("I'm here at alloc_heap_pages, scrubing pages\n");
		}
    ...
    unsigned int memflags)
{
    ...
    if ( !(memflags & MEMF_no_refcount) )
    {
        ...
        {
		printk("I'm here at assign_pages, about to go out, d->tot_pages:%d, order:%u, d->max_pages:%d\n", d->tot_pages, (1 << order), d->max_pages);
    ...
    struct domain *d, unsigned int order, unsigned int memflags)
{
    ...
    {
	printk("I'm here at alloc_domheap_pages, return null 1\n");
        return NULL;
    }
    ...
    if ( (pg == NULL) &&
         ((memflags & MEMF_no_dma) ||
          ((pg = alloc_heap_pages(MEMZONE_XEN + 1, zone_hi, order,
                                  memflags, d)) == NULL)) )
    {
	printk("I'm here at alloc_domheap_pages, return null 2\n");
         return NULL;
    }

    if ( d && !(memflags & MEMF_no_owner))
    {
	ret = assign_pages(d, pg, order, memflags);
        if (ret){
	    printk("I'm here at alloc_domheap_pages, return null 3, ret: %d\n", ret);
            return NULL;
	}
    }
