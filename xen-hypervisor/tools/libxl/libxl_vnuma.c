                                            ...
                                            libxl__domain_build_state *state)
{
	...
	fprintf(stderr, "libxl__vnuma_build_vmemrange_pv_generic: p->memkb: %ld\n", p->memkb);
                                     ...
                                     struct xc_dom_image *dom)
{
    ...
    for (nid = 0; nid < b_info->num_vnuma_nodes; nid++) {
		...
		fprintf(stderr, "libxl__vnuma_build_vmemrange_hvm: p->memkb: %ld, b_info->video_memkb: %ld\n", p->memkb, b_info->video_memkb);
                rc = ERROR_INVAL;
                goto out;
            }
