...
retry_transaction:
    ...
    char* memmax = libxl__xs_read(gc, t, GCSPRINTF("%s/memory/static-max", dom_path));
    printf("memmax: %s before libxl__xs_writev writes on ents in path %s\n", memmax, GCSPRINTF("%s/memory/static-max", dom_path));
    printf("ents[0]:%s, ents[1]:%s\n", ents[0], ents[1]);
    ...
    memmax = libxl__xs_read(gc, t, GCSPRINTF("%s/memory/static-max", dom_path));
    printf("memmax: %s after libxl__xs_writev writes on ents\n", memmax);
