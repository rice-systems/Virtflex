                                   ...
                                   libxl__domain_create_state *dcs)
{
    ...
    if(d_config->b_info.max_memkb)
    	printf("I'm here at initiate_domin_create, maxmem=%lu\n",d_config->b_info.max_memkb);
                            ...
                            const libxl_asyncprogress_how *aop_console_how)
{
    ...
    int rc;
    if(d_config->b_info.max_memkb)
    	printf("I'm here at do_domain_create, maxmem=%lu\n",d_config->b_info.max_memkb);
...
static int vnuma_write_to_xs(libxl_ctx *ctx, int domid, 
				int num_nodes, struct vnode_info* vnode_infos){
    ...
    dom_path = libxl__xs_get_dompath(gc, domid);
    t = xs_transaction_start(ctx->xsh);
    libxl__xs_mknod(gc, t, GCSPRINTF("%s/numa", dom_path), rwperm, ARRAY_SIZE(rwperm));
    libxl__xs_mknod(gc, t, GCSPRINTF("%s/numa/node", dom_path), rwperm, ARRAY_SIZE(rwperm));
    xs_write(ctx->xsh, t, GCSPRINTF("%s/numa/topo_change", dom_path),
                                                "0", strlen("0"));

    for(int i=0; i < num_nodes; i++){
        libxl__xs_mknod(gc, t, GCSPRINTF("%s/numa/node/%d", dom_path, i),
                                                rwperm, ARRAY_SIZE(rwperm));
        libxl__xs_mknod(gc, t, GCSPRINTF("%s/numa/node/%d/vcpu", dom_path, i),
                                                rwperm, ARRAY_SIZE(rwperm));
        // construct buf w.r.t pnode
        snprintf(buf, sizeof(buf), "%d", vnode_infos[i].pnode);
        xs_write(ctx->xsh, t, GCSPRINTF("%s/numa/node/%d/pnode", dom_path, i),
                                                buf, strlen(buf));
        // construct buf wrt capacity
        snprintf(buf, sizeof(buf), "%lu", vnode_infos[i].capacity);
        xs_write(ctx->xsh, t, GCSPRINTF("%s/numa/node/%d/capacity", dom_path, i),
                                                buf, strlen(buf));
        // construct buf wrt target
        snprintf(buf, sizeof(buf), "%lu", vnode_infos[i].target);
        xs_write(ctx->xsh, t, GCSPRINTF("%s/numa/node/%d/target", dom_path, i),
                                                buf, strlen(buf));
        for (int j=0; j < vnode_infos[i].num_vcpus; j++){
            xs_write(ctx->xsh, t, 
		GCSPRINTF("%s/numa/node/%d/vcpu/%d", dom_path, i, vnode_infos[i].vcpu[j]),
                                                "online", strlen("online"));
        }
    }
    printf("In vnuma_write_to_xs, finished writing vnode info to xenstore\n");
			    ...
			    struct vnode_info_parameter* vnode_para)
{
    ...
    ret = do_domain_create(ctx, d_config, domid, -1, -1, NULL,
                            ao_how, aop_console_how);
    if (vnode_para){
    	vnuma_write_to_xs(ctx, *domid, vnode_para->num_nodes, vnode_para->vnode_infos);
	printf("In libxl_domain_create_new, returned from vnuma_write_to_xs\n");
    }
    return ret;
}
