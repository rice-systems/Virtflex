                       ...
                       unsigned int *vcpu_to_vnode)
{
    ...
    vnuma_topo.pad = 0;
    //fprintf(stderr, "DEBUG: xc_domain_getvnuma: before do_memory_op(), topo.nr_vnodes:%d, topo.nr_vcpus:%d, topo.nr_vranges:%d \n",vnuma_topo.nr_vnodes, vnuma_topo.nr_vcpus, vnuma_topo.nr_vmemranges);

    rc = do_memory_op(xch, XENMEM_get_vnumainfo, &vnuma_topo,
                      sizeof(vnuma_topo));
    //if (rc)
    //fprintf(stderr, "DEBUG: xc_domain_getvnuma: do_memory_op() returns non zero, rc = %d \n", rc);
