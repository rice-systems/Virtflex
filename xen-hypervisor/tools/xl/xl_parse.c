...
static void parse_vnuma_config(const XLU_Config *config,
				...
				struct vnode_info_parameter* vnode_para)
{
    ...
    if (vnode_para){
	vnode_para->num_nodes = num_vnuma;
	vnode_para->vnode_infos = xcalloc(num_vnuma, sizeof(struct vnode_info));
    }
        ...
        for (conf_count = 0;
            ...
            if (xlu_cfg_value_type(conf_option) == XLU_STRING) {
                ...
                if (!strcmp("pnode", option)) {
                    ...
                    libxl_defbool_set(&b_info->numa_placement, false);
		    if(vnode_para){
		    	vnode_para->vnode_infos[i].pnode = val;
		    } 
                    ...
                    max_memkb += p->memkb;
		    if(vnode_para){
		    	vnode_para->vnode_infos[i].capacity = p->memkb;
			vnode_para->vnode_infos[i].target = p->memkb;
		    } 
		    ...
		    if (vnode_para){
		        ...
		        vnode_para->vnode_infos[i].vcpu = 
					xcalloc(vcpu_cnt, sizeof(int));
			vnode_para->vnode_infos[i].num_vcpus = vcpu_cnt;
			vcpu_cnt = 0;
		    }
                    for (j = 0; j < len; j++) {
                        parse_range(cpu_spec_list[j], &s, &e);
                        for (; s <= e; s++) {
                             ...
                             */
			    if (vnode_para){
				vnode_para->vnode_infos[i].vcpu[vcpu_cnt] = s;
			    }
			    vcpu_cnt++;
			    printf("In parse_vnuma_config, store %ld \tin vnode_info %d\n", s, i);
    ...
    /*
	The following code is added to write vnuma info into xenstore.
	The structure of the vnuma xenstore directory
		/local/domain/#/memory/node/#/:
						pnode
						vcpu
							0: online
							1: offline
...
//
//    rwperm[0].id = domid;
//    rwperm[0].perms = XS_PERM_READ;   
//
//    dom_path = libxl__xs_get_dompath(gc, domid);
//    t = xs_transaction_start(ctx->xsh);
//    libxl__xs_mknod(gc, t, GCSPRINTF("%s/memory/node", dom_path), rwperm, ARRAY_SIZE(roperm));
//    
//    for(int i=0; i < num_nodes; i++){
//	libxl__xs_mknod(gc, t, GCSPRINTF("%s/memory/node/%d", dom_path, i), 
//						rwperm, ARRAY_SIZE(roperm));
//	libxl__xs_mknod(gc, t, GCSPRINTF("%s/memory/node/%d/vcpu", dom_path, i), 
//						rwperm, ARRAY_SIZE(roperm));
//	// construct buf w.r.t pnode
//	snprintf(buf, sizeof(buf), "%d", vnode_infos[i].pnode);
//	xs_write(ctx->xsh, t, GCSPRINTF("%s/memory/node/%d/pnode", dom_path, i), 
//						buf, strlen(buf));
//	// construct buf wrt capacity
//	snprintf(buf, sizeof(buf), "%ul", vnode_infos[i].capacity);
//	xs_write(ctx->xsh, t, GCSPRINTF("%s/memory/node/%d/capacity", dom_path, i),
//                                                buf, strlen(buf));
//	// construct buf wrt target
//	snprintf(buf, sizeof(buf), "%ul", vnode_infos[i].target);
//	xs_write(ctx->xsh, t, GCSPRINTF("%s/memory/node/%d/target", dom_path, i),
//                                                buf, strlen(buf));
//	for (int j=0; j < vnode_infos[i].num_vcpus; j++){
//	    xs_write(ctx->xsh, t, GCSPRINTF("%s/memory/node/%d/vcpu/%d", dom_path, i, j),
		       ...
		       struct vnode_info_parameter* vnode_para)
{
    ...
    if(b_info)
        printf("I'm here at parse_config_data before parse_vnuma_config, maxmem=%lu\n",b_info->max_memkb);
    parse_vnuma_config(config, b_info, vnode_para);
    if(b_info)
        printf("I'm here at parse_config_data after parse_vnuma_config, maxmem=%lu\n",b_info->max_memkb);
    ...
    if(b_info)
        printf("I'm here at parse_config_data 1, maxmem=%lu\n",b_info->max_memkb);
...
#undef parse_extra_args
    ...
    if(b_info)
        printf("I'm here at parse_config_data 2, maxmem=%lu\n",b_info->max_memkb);
