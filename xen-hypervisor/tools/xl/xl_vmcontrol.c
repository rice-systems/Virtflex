...
//static int vnuma_write_to_xs(int domid, int num_nodes, struct vnode_info* vnode_infos){
//
//    char *dom_path;
...
//    libxl__xs_mknod(gc, t, GCSPRINTF("%s/memory/node", dom_path), rwperm, ARRAY_SIZE(rwperm));
//
...
//        snprintf(buf, sizeof(buf), "%lu", vnode_infos[i].capacity);
//        xs_write(ctx->xsh, t, GCSPRINTF("%s/memory/node/%d/capacity", dom_path, i),
//                                                buf, strlen(buf));
//        // construct buf wrt target
//        snprintf(buf, sizeof(buf), "%lu", vnode_infos[i].target);
...
int create_domain(struct domain_create *dom_info)
{
    ...
    // vnode_info_para is for interal use between parse_numa_config and vnuma_write_to_xs
    struct vnode_info_parameter vnode_info_para;
    vnode_info_para.vnode_infos = NULL;
    ...
    libxl_domain_config_init(&d_config);
    if(d_config.b_info.max_memkb)
        printf("I'm here at create_domain, maxmem=%lu\n",d_config.b_info.max_memkb);
    if (restoring) {
...
#define WITH_OPTDATA(amt, body)                                 \
    ...
    if(d_config.b_info.max_memkb)
        printf("I'm here at create_domain 1, maxmem=%lu\n",d_config.b_info.max_memkb);

    if (config_in_json) {
        libxl_domain_config_from_json(ctx, &d_config,
                                      (const char *)config_data);
    } else {
	printf("called parse_config_data\n");
        parse_config_data(config_source, config_data, config_len, &d_config, &vnode_info_para);
    }
        if(d_config.b_info.max_memkb)
        printf("I'm here at create_domain 2, maxmem=%lu\n",d_config.b_info.max_memkb);
...
start:
    ...
    if ( restoring ) {
        ...
        ret = libxl_domain_create_new(ctx, &d_config, &domid,
                                      0, autoconnect_console_how, &vnode_info_para);
    }

    //vnuma_write_to_xs(domid, vnode_info_para.num_nodes, vnode_info_para.vnode_infos);
    
    // free pointers in vnode_info_para and vnode_infos
    for (int j = 0; j < vnode_info_para.num_nodes; j++){
	if( 	vnode_info_para.vnode_infos && 
		vnode_info_para.vnode_infos[j].vcpu){
		free(vnode_info_para.vnode_infos[j].vcpu);	
	}
    }
    if (vnode_info_para.vnode_infos)
        free(vnode_info_para.vnode_infos);
    printf("In create_domain, finished freeing pointers in vnode_info\n");
