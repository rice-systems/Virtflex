...
void __init arch_xen_balloon_init(struct resource *hostmem_resource)
{
	...
	printk("I'm in arch_xen_balloon_init!\n");
	...
	for (i = 0; i < memmap.nr_entries; i++) {
		printk("entry %d, addr: %llu, size: %llu, type: %d\n",i,xen_e820_table->entries[i].addr,xen_e820_table->entries[i].size, xen_e820_table->entries[i].type);
